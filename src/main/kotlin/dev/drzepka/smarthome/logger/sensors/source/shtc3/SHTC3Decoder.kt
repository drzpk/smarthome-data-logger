 ...
 */
object SHTC3Decoder : DataDecoder<Pair<MacAddress, ByteArray>> {
    private val log by Logger()

    override fun decode(item: Pair<MacAddress, ByteArray>): Collection<Measurement> {
        val buffer = ByteBuffer.wrap(item.second)
        if (!checkCrc(item.second))
            return emptyList()
