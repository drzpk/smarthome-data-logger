...
internal class SofarDecoderTest {
    ...
    private val decoder = SofarDecoder("test-mac")
    ...
    fun `should map mac and time`() {
        ...
        then(m.time).isEqualTo(data.date)
    }

    @Test
    fun `should map current power`() {
        val data = sofarData(currentPower = 3500)
    ...
    fun `should convert energy today from Wh to kWh`() {
        val data = sofarData(energyToday = 12500)
    ...
    fun `should convert energy total from Wh to kWh`() {
        val data = sofarData(energyTotal = 5_000_000)
    ...
    fun `should use global frequency for all phases`() {
        val data = sofarData(frequency = 50.01f)
    ...
    private fun sofarData(
        ...
        date = Instant.parse("2024-06-01T10:00:00Z"),
        energyToday = energyToday,
        energyTotal = energyTotal,
        currentPower = currentPower,
        frequency = frequency,
        generationHoursToday = 3.5f,
        generationHoursTotal = 1200,
