...
internal class ConnectionErrorTrackerTest {
    ...
    fun `should log errors below threshold`() {
        ...
        then(tracker.recordFailure()).isTrue
        then(tracker.recordFailure()).isTrue
    }
    ...
    fun `should suppress logging at threshold and start throttle countdown`() {
        ...
        tracker.recordFailure()
        val shouldLog = tracker.recordFailure()
    ...
    fun `should suppress logging above threshold`() {
        ...
        repeat(3) { tracker.recordFailure() }
    ...
    fun `should skip again after failed retry when throttled`() {
        ...
        tracker.recordFailure()   // retry failed
    ...
    fun `should cap skip count at maxSkipCount`() {
        ...
        repeat(3) { tracker.recordFailure() }
        skipCount(tracker)
        tracker.recordFailure() // 2 * 4 = 8, capped at 6
    ...
    fun `should reset skip count to initial on recovery`() {
        ...
        repeat(3) { tracker.recordFailure() }
        skipCount(tracker)
        tracker.recordFailure() // backoff applied: currentSkipCount = 4
        skipCount(tracker)
        tracker.recordSuccess()
    ...
    ) = ErrorTracker("test", errorThreshold, throttleSkipCount, backoffFactor, maxSkipCount)

    private fun skipCount(tracker: ErrorTracker): Int {
        var count = 0
